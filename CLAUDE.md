...
- **Current Track**: **AI Quant Terminal** (consumer stock terminal, Terminal mode default / Research mode retained) — governed by `AI_QUANT_TERMINAL_PRODUCT_SIMPLIFICATION_PROPOSAL.md`. Real-time A-share quotes are live via a **public, undocumented, unlicensed** endpoint (`src/data/providers/sina_quote_provider.py`): no SLA, delayed rather than tick-level, fine for research/personal use, **a licensed vendor is required before commercial distribution**. REAL and DEMO data must never appear on the same page. The prior track is **AI Quant Research Analyst** — governed by `AI_QUANT_RESEARCH_ANALYST_ARCHITECTURE_PROPOSAL.md` (revision 2) §11. Phase 9 is complete; this track is deliberately **unnumbered** and **no "Phase 10" is ever to be created**. Volatile state — current HEAD, test counts, and the exact next action — lives in `.claude/CURRENT_STATE.md` and `.claude/HANDOFF.md` by design (§5); it is intentionally not duplicated into this file, which would only guarantee the two disagree.
