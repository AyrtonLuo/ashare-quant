# Final Research Integrity Certification Document (Phase 7G-CG)

## 1. Executive Purpose & Scope
This document provides final research integrity certification for the A-Share Quantitative Platform (`ashare-quant`). It certifies point-in-time (PIT) temporal isolation, dataset locking, revision control, research reproducibility, zero-secret security auditing, and live provider preflight gates.
...
| **Live Network Provider Execution** | **SKIPPED (UNAVAILABLE)** | `TUSHARE_TOKEN` absent in env. Gracefully skipped per Section 1. |
...
| **Dataset Manifest & SHA-256** | **VERIFIED** | `real_provider_dataset_v4` / `ds_live_v4.0` checksum generated & verified. |
| **PIT Snapshot Isolation** | **VERIFIED** | `snap_live_2022_05_02_v4` verified with 0 future data leaks. |
...
| **Research Run Identity** | **VERIFIED** | `real_provider_research_run_v4` captures complete parameter & code version hashes. |
