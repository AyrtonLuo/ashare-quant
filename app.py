...
# =============================================================================
    ...
    with st.expander("📖 解决方案指南：如何教别人在网页版/本地连接富途牛牛模拟盘？", expanded=False):
        st.markdown("""
### 🌐 教程：让其他人在网页版上连接富途牛牛模拟盘

---

#### 🌐 途径一：如何在云端网页版上直连富途模拟盘？（无需部署代码）

若其他人想直接通过浏览器访问云端网页 (`streamlit.app`) 对接富途模拟盘，请按以下步骤操作：
...
3. **第三步（获取公网 IP 或域名）**：
   ...
   - **跨公网/外网下**：在终端运行免费穿透工具（如运行命令 `ngrok tcp 11111`），获取穿透公网域名（例：`0.tcp.ngrok.io`）；
4. **第四步（网页填入并连接）**：
   在上方 **【网关 IP / 公网域名】** 中填入得到的 IP 或域名，开启开关即可在云端网页版上直接查看与下发富途模拟盘挂单！

---

#### 💻 途径二：若跨网防火墙限制无法连接，手把手教如何在本地 1 分钟运行

若因公网防火墙或网络隔离导致云端网页无法连接，可教他们在自己电脑本地一键运行：
...
2. **第二步（一键启动本地网页）**：
   ...
   ```
3. **第三步（秒级自动识别直连）**：
   浏览器会自动打开 **`http://localhost:8501`**。因为网页与 Futu OpenD 都在同台电脑运行，系统会 100% 自动识别并秒级成功连上富途牛牛模拟盘！
""")
